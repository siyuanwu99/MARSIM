...
int minus_twopointcloud(pcl::PointCloud<pcl::PointXYZ>& cloud_input,
  ...
  for (int i = 0; i < cloud_input.points.size(); i++) {
    if (minus_kdtree.radiusSearch(cloud_input.points[i], 0.6, pointIdxRadiusSearch,
...
int main(int argc, char** argv) {
  ...
  //      Eigen::Vector3d pt(cloud_temp.points[i].x, cloud_temp.points[i].y,
  //      cloud_temp.points[i].z); cloud.points.push_back(cloud_temp.points[i]);
  //  }
  ...
  // string file_name2 =
  // "/home/jackykong/motionplanning/FUEL_ws/src/FUEL/meshmap/hku_demo_color.ply"; pcl::PLYReader
  // reader; pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_ply;
  ...
  ROS_INFO("Map bound: x=%f,%f, y=%f,%f, z=%f,%f", global_mapmin.x, global_mapmax.x,
           global_mapmin.y, global_mapmax.y, global_mapmin.z, global_mapmax.z);
  ...
  if (add_boundary == 1) {
    ...
    // draw six plane
    cloud_boundary.width = (box_x + 1) * (box_y + 1) * (box_z + 1) -
                           (box_x - 1) * (box_y - 1) * (box_z - 1);  // points number
  ...
  // 创建用于最近邻搜索的KD-Tree pcl::PointCloud<PointT> mls_point;    //输出MLS
  ...
  //   创建用于最近邻搜索的KD-Tree normalEstimation.setSearchMethod(tree);
  ...
  //   // 输出法线
  //   std::cout<<"normals: "<<normals->size()<<", "<<"normals fields:
  //   "<<pcl::getFieldsList(*normals)<<std::endl;
  //   //
  ...
  //   pcl::PointCloud<pcl::PointNormal>); pcl::concatenateFields(*cloud_smoothed, *normals,
  //   *cloud_with_normals);
  //   //
  ...
  //   //设置该参数为true保证法线朝向一致，设置为false的话不会进行法线一致性检查

  //   gp3.setInputCloud (cloud_with_normals);     //设置输入点云为有向点云
  //   gp3.setSearchMethod (tree2);   //设置搜索方式
  //   gp3.reconstruct (triangles);  //重建提取三角化
  //   pcl::io::saveOBJFile("/home/jackykong/motionplanning/FUEL_ws/src/Exploration_sim/uav_simulator/map_generator/resource/result.obj",
  //   triangles); std::cout<<" Out put finished"<<std::endl;

  // string objPath =
  // "/home/jackykong/motionplanning/FUEL_ws/src/Exploration_sim/uav_simulator/map_generator/resource/result.obj";
  // //当前目录下的obj文件
  // //读取
  ...
  // pcl::visualization::PCLVisualizer("3D Viewer")); viewer->setBackgroundColor(0, 0, 0);  //
  ...
  // write files
  //  std::string
  //  pkg_path("/home/jackykong/motionplanning/FUEL_ws/src/Exploration_sim/uav_simulator/map_generator/resource");
  ...
  // ros::Publisher minus_cloud_pub = node.advertise<sensor_msgs::PointCloud2>("/minus_cloud", 10,
  // true); pcl::PointCloud<pcl::PointXYZ> cloud_minus, cloud_output; status =
  // pcl::io::loadPCDFile<pcl::PointXYZ>("/home/jackykong/motionplanning/FUEL_ws/src/Exploration_sim/octomap_mapping/octomap_server/data/FUEL_WATERTRE03NEW2.pcd",
  // cloud_minus); if (status == -1)
  // {
  //   cout << "can't read file." << endl;
  //   return -1;
  // }
