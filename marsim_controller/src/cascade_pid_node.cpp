...
void run_control(const ros::TimerEvent& event) {
  if (first_odom_flag == 1) {
    ...
    Eigen::Quaterniond quaternion_temp(
        current_odom.pose.pose.orientation.w, current_odom.pose.pose.orientation.x,
        current_odom.pose.pose.orientation.y, current_odom.pose.pose.orientation.z);
    Vector3d current_pos, current_vel;
    current_pos << current_odom.pose.pose.position.x, current_odom.pose.pose.position.y,
        current_odom.pose.pose.position.z;
    current_vel << current_odom.twist.twist.linear.x, current_odom.twist.twist.linear.y,
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber odom_sub =
      n.subscribe("odom", 100, Odom_callback, ros::TransportHints().tcpNoDelay());
  ros::Subscriber cmd_sub =
      n.subscribe("cmd_pose", 100, cmd_callback, ros::TransportHints().tcpNoDelay());
