...
class CascadePID {
 private:
  ...
  double             k_T =
      0.07 * (3 * 0.062) * k_F;  // fixed parameter for calculate motor torque from motor speed
...
CascadePID::~CascadePID() { myfile.close(); }

void CascadePID::setdroneid(int id) {
  droneid = id;

  pkg_path = ros::package::getPath("marsim_controller");
  pkg_path.append("/data/log_" + std::to_string(droneid) + ".txt");
  std::cout << "\nFound pkg_path = " << pkg_path << std::endl;
  myfile.open(pkg_path.c_str(), std::ios_base::out);
}

void CascadePID::setrate(double control_rate) { dt = 1.0 / control_rate; }

void CascadePID::Quat2EulerAngle(const Quaterniond& q_input,
                                 double&            roll,
                                 double&            pitch,
                                 double&            yaw) {
...
void CascadePID::setInternal(double m, Matrix3d I, double arm, double kF) {
  ...
  motor_pos << arm_length * sqrt(2) / 2.0, -arm_length * sqrt(2) / 2.0, 0,
      -arm_length * sqrt(2) / 2.0, arm_length * sqrt(2) / 2.0, 0, arm_length * sqrt(2) / 2.0,
      arm_length * sqrt(2) / 2.0, 0, -arm_length * sqrt(2) / 2.0, -arm_length * sqrt(2) / 2.0, 0;
...
void CascadePID::setParam(double stable_time, double damping_ratio) {
  ...
  Angle_Kd << 2 * damping_ratio * angle_wn, 2 * damping_ratio * angle_wn,
      2 * damping_ratio * angle_wn;
...
void CascadePID::RunController() {
  ...
  double a_des_z =
      (KP_Z * Pos_error(2) + KD_Z * Vel_error(2) + KI_Z * z_intergral_val + Acc_des(2));
  ...
  if (new_z_body.dot(z_body) == 1.0) {
    ...
    new_z_body_inv << 0, -rotation_vec(2), rotation_vec(1), rotation_vec(2), 0, -rotation_vec(0),
    ...
    I << 1, 0, 0, 0, 1, 0, 0, 0, 1;
    R_des = I + sin(rotation_angle) * new_z_body_inv +
            (1 - cos(rotation_angle)) * new_z_body_inv * new_z_body_inv;
  }
  ...
  // double tilt_angle_des = asin((mass* sqrt(acc_x_des*acc_x_des + acc_y_des*acc_y_des))/
  // (F_des*cos(theta_des)));
  ...
  //             // << " theta_des = " << theta_des << " tilt_angle_des = " << tilt_angle_des <<
  //             std::endl;
  //             << "euler angle = " << euler_angle(0) << " " << euler_angle(1) << " " <<
  //             euler_angle(2) << std::endl;
  ...
  Matrix3d R_z;
  R_z << cos(Yaw_des), -sin(Yaw_des), 0, sin(Yaw_des), cos(Yaw_des), 0, 0, 0, 1;
  ...
  // // d_Angle_error(2) = std::max(std::min(d_Angle_error(2), max_angvel_error),
  // -max_angvel_error); Angle_last_error = Angle_error;
  ...
  // std::cout << "rotation vector = " << rotation_vec_des.axis().transpose()  << ", rotation angle
  // = "<< rotation_vec_des.angle()*(180/M_PI) << std::endl;
  ...
  Torque_des(0) = (Angle_Kp(0) * rotation_vec_des.axis()(0) * rotation_vec_des.angle() +
                   Angle_Kd(0) * d_Angle_error(0));
  Torque_des(1) = (Angle_Kp(1) * rotation_vec_des.axis()(1) * rotation_vec_des.angle() +
                   Angle_Kd(1) * d_Angle_error(1));
  Torque_des(2) = (Angle_Kp(2) * rotation_vec_des.axis()(2) * rotation_vec_des.angle() +
                   Angle_Kd(2) * d_Angle_error(2));
  Torque_des    = Internal_mat * Torque_des;
  ...
  for (int i = 0; i < 4; i++) {
    // sqrt cause nan
    RPM_output(i) = F_des / (4.0 * k_F) + Torque_des(0) / (4.0 * Mixermatrix_rpm2torque(1, i)) +
  ...
  // record log
  myfile << Pos_des(0) << " " << Pos_des(1) << " " << Pos_des(2) << " " << current_pos(0) << " "
         << current_pos(1) << " " << current_pos(2) << " " << Vel_des(0) << " " << Vel_des(1) << " "
         << Vel_des(2) << " " << current_vel(0) << " " << current_vel(1) << " " << current_vel(2)
         << " " << angle_des(0) << " " << angle_des(1) << " " << Yaw_des << " " << euler_angle(0)
         << " " << euler_angle(1) << " " << Yaw_cur << " " << Angle_error(0) << " "
         << Angle_error(1) << " " << Angle_error(2) << " " << Acc_des(0) << " " << Acc_des(1) << " "
         << Acc_des(2) << " " << Torque_des(0) << " " << Torque_des(1) << " " << Torque_des(2)
         << " " << new_z_body(0) << " " << new_z_body(1) << " " << new_z_body(2) << " "
         << RPM_output(0) << " " << RPM_output(1) << " " << RPM_output(2) << " " << RPM_output(3)
         << std::endl;
}

Vector3d CascadePID::getTorquedes() { return Torque_des; }

Vector4d CascadePID::getRPMoutputs() { return RPM_output; }
