...
void dynamics::step(double dt) {
  ...
  // ouler equation
  w_body =
      w_body + dt * Internal_mat.inverse() * (Torque_body - w_body.cross(Internal_mat * w_body));

  // from angular velocity to quat
  Eigen::Matrix4d q_transmat;
  Vector4d        quat_temp;
  quat_temp = q;
  q_transmat << quat_temp(0), -quat_temp(1), -quat_temp(2), -quat_temp(3), quat_temp(1),
      quat_temp(0), -quat_temp(3), quat_temp(2), quat_temp(2), quat_temp(3), quat_temp(0),
      -quat_temp(1), quat_temp(3), -quat_temp(2), quat_temp(1), quat_temp(0);
...
Vector3d dynamics::getPos() { return pos; }

Vector3d dynamics::getVel() { return velocity; }

Vector3d dynamics::getAcc() { return acceleration; }
...
Vector3d dynamics::getAngularVel() { return w_body; }

Matrix3d dynamics::getR() { return R_body2world; }
